(function( $ ){
  var methods = {
      init: function(options){
          ...
          $options.each(function(i,el){
                ...
                dropdown.append('<img style="width:48px;" onclick="jQuery(\'select[name=' + selectName + ']\').val(\''+ $(el).val() + '\').ImageSelect(\'close\').ImageSelect(\'update\',{src:\''+ $(el).text() + '\'});" src="' + $(el).text() + '"/>');
          });
  ...
  $.fn.ImageSelect = function(method,options) {
    ...
    return this.each(function() {
        if ( methods[method] ) {
            var ret = methods[method].apply( this, Array(settings)); 
            $(".jqis_dropdown").css("overflow","scroll");
            return ret;
        } else if ( typeof method === 'object' || ! method ) {
            var ret =  methods.init.apply( this, Array(settings) );
            return ret;
        } else {
